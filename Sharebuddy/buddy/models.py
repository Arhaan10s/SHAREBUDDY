...
class Listings(models.Model):
    ...
    description = models.CharField(max_length=2048, help_text="A longer description that lets users know more about what you're selling!")
    starting_bid = models.DecimalField(max_digits=8, decimal_places=2, help_text="What is the starting price you want to sell your product for? All prices are in USD!")
    image_url = models.CharField(max_length=2048, blank=True, help_text="This is not required, but if you want to include an image for your product, please include an online url for it here!")
    category = models.CharField(max_length=64, blank=True, help_text="Again, not required, but this could help your product gain the spotlight it deserves! Examples of categories include Fashion, Toys Electronics, Home, etc.")
...
class Bids(models.Model):
    ...
    value_offer = models.DecimalField(max_digits=8, decimal_places=2, help_text="How much are you willing to pay for  this item?")
...
class Comments(models.Model):
    author = models.ForeignKey(User, on_delete=models.CASCADE, related_name="comments",default=True)
    content = models.CharField(max_length=2048,default=True)
